"""
Hard Skills Scorer
Evalua competencias tecnicas usando TF-IDF + Jaccard Similarity
"""
...
def calculate_tfidf_similarity(cv_skills: List[str], required_skills: List[str]) -> float:
    """
    Calcula similitud semantica usando TF-IDF
    Util para detectar skills relacionados (ej: "Machine Learning" vs "ML")
    ...
    if not SKLEARN_AVAILABLE:
        # Fallback: usar Jaccard si sklearn no esta disponible
        return calculate_jaccard_similarity(set(cv_skills), set(required_skills))
...
def calculate_hard_skills_score(
    ...
    """
    Calcula score de hard skills combinando matching exacto y semantico

    Estrategia:
    1. Normalizar todos los skills
    2. Matching exacto (Jaccard) con skills requeridos
    3. Matching semantico (TF-IDF) para detectar relacionados
    ...
    # Normalizar skills
    cv_normalized = set([normalize_skill(s) for s in cv_skills]) if cv_skills else set()
    required_normalized = set([normalize_skill(s) for s in required_skills]) if required_skills else set()
    preferred_normalized = set([normalize_skill(s) for s in preferred_skills]) if preferred_skills else set()

    # Si no hay skills requeridos, retornar score perfecto
    ...
    WEIGHT_SEMANTIC = 0.25         # 25% - Similitud semantica
    WEIGHT_PREFERRED = 0.15        # 15% - Match con preferidos
    WEIGHT_BREADTH = 0.10          # 10% - Amplitud de skills (bonus por tener mas)

    # Componente 1: Match exacto con requeridos
    score_required = required_match_ratio

    # Componente 2: Similitud semantica (captura relacionados)
