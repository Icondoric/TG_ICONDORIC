  ...
  <AppLayout>
    <div class="min-h-screen bg-gray-50 py-8">
      <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div class="flex justify-center mb-8">
            <div class="flex items-center">
              <div :class="[
                ...
                step >= 1 ? 'bg-emi-navy-500' : 'bg-gray-300'
              ]">1</div>
              <div :class="['w-16 h-1', step >= 2 ? 'bg-emi-navy-500' : 'bg-gray-300']"></div>
              <div :class="[
                'w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold',
                step >= 2 ? 'bg-emi-navy-500' : 'bg-gray-300'
              ]">2</div>
              <div :class="['w-16 h-1', step >= 3 ? 'bg-emi-navy-500' : 'bg-gray-300']"></div>
              <div :class="[
                'w-10 h-10 rounded-full flex items-center justify-center text-white font-semibold',
          ...
          <div v-if="step === 1">
            <div
              ...
              :class="[
                ...
                isDragging ? 'border-emi-navy-500 bg-emi-navy-50' : 'border-gray-300 hover:border-gray-400'
              ]"
            >
              ...
              <label class="mt-2 cursor-pointer">
                <span class="text-emi-navy-500 hover:text-emi-navy-600 font-medium">selecciona un archivo</span>
                <input
                  type="file"
                  class="hidden"
                  accept=".pdf"
            ...
            <div v-if="selectedFile" class="mt-6 p-4 bg-gray-50 rounded-lg">
              ...
              <button
                @click="uploadFile"
                :disabled="uploading"
                class="mt-4 w-full btn-emi-primary disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div v-if="step === 3">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div class="bg-blue-50 rounded-lg p-4 text-center">
                <p class="text-2xl font-bold text-emi-navy-500">{{ extractionSummary.hard_skills_count }}</p>
        ...
        <!-- Info Card -->
        <div class="mt-6 bg-emi-navy-50 rounded-lg p-6">
          <h3 class="font-medium text-emi-navy-800 mb-2">Como funciona</h3>
          <ul class="text-sm text-emi-navy-700 space-y-2">
            <li class="flex items-start">
              <span class="flex-shrink-0 w-5 h-5 bg-emi-navy-200 rounded-full flex items-center justify-center text-xs font-medium text-emi-navy-800 mr-2">1</span>
              Sube tu CV en formato PDF
            </li>
            <li class="flex items-start">
              <span class="flex-shrink-0 w-5 h-5 bg-emi-navy-200 rounded-full flex items-center justify-center text-xs font-medium text-emi-navy-800 mr-2">2</span>
              Nuestro sistema extrae tus competencias automaticamente
            </li>
            <li class="flex items-start">
              <span class="flex-shrink-0 w-5 h-5 bg-emi-navy-200 rounded-full flex items-center justify-center text-xs font-medium text-emi-navy-800 mr-2">3</span>
