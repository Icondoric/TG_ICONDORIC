...
const loadProfiles = async () => {
    try {
        await mlStore.loadProfiles(includeInactive.value, selectedSector.value || null)
    } catch (error) {
...
const deleteProfile = async () => {
    ...
    try {
        await mlStore.deleteProfileAction(deleteConfirm.value.id)
        deleteConfirm.value = null
    } catch (error) {
...
const activateProfile = async (profileId) => {
    try {
        await mlStore.activateProfileAction(profileId)
    } catch (error) {
...
<template>
    <div class="max-w-7xl mx-auto py-8 px-4">
            ...
            <div class="flex flex-col md:flex-row gap-4">
                ...
                <div>
                    ...
                    >
                        ...
                        <option v-for="sector in mlStore.sectors" :key="sector" :value="sector">
                            {{ sector }}
                        </option>
                    </select>
                </div>
        ...
        <!-- Loading -->
        <div v-if="mlStore.isLoadingProfiles" class="bg-white rounded-xl shadow-md p-8 text-center">
            <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
            <p class="mt-4 text-slate-500">Cargando perfiles...</p>
        </div>

        <!-- Error -->
        <div v-else-if="mlStore.profilesError" class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <p class="text-red-700">{{ mlStore.profilesError }}</p>
        </div>
        ...
        <div v-else class="space-y-4">
            ...
            <div class="text-sm text-slate-500 mb-2">
                Mostrando {{ filteredProfiles.length }} de {{ mlStore.profiles.length }} perfiles
            </div>
